    ...
    <div className="app-shell">
      ...
      <div className="main-shell">
        ...
        <main id="main-content">
          <div className="page-heading"><div><p className="eyebrow">{page === 'Dashboard' ? en.setup : en.localWorkspace}</p><h1 ref={heading} tabIndex={-1}>{page === 'Dashboard' ? en.overview : en.navigation[page]}</h1><p>{page === 'Dashboard' ? en.intro : page === 'Settings' ? en.settingsDetail : page === 'Projects' ? en.projects.intro : page === 'Models' ? en.models.pageDetail : en.tagline}</p></div><span className="phase-badge">{en.foundation}</span></div>
          ...
          </> : page === 'Projects' ? <Projects /> : page === 'Dataset' ? <Dataset /> : page === 'Models' ? <Models baseModel={info?.base_model ?? null} /> : page === 'Cameras' ? <Cameras /> : page !== 'Settings' ? <section className="empty-state"><span className="empty-icon"><FolderOpen size={32} /></span><span className="planned">{en.planned}</span><h2>{en.futureTitle}</h2><p>{en.pageDetails[page]}</p><button className="primary-button" onClick={() => setPage('Dashboard')}>{en.back}<ArrowRight size={17} /></button></section> : null}
